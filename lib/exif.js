...
async function writeExifImg (media, metadata) {
    ...
    if (metadata.packname || metadata.author) {
        ...
        const json = { "sticker-pack-id": `https://github.com/PROFESSOR-CJ7/Professor-XMD`, "sticker-pack-name": metadata.packname, "sticker-pack-publisher": metadata.author, "emojis": metadata.categories ? metadata.categories : [""] }
...
async function exifAvatar(buffer, packname, author, categories = [''], extra = {}) {
  ...
  const img = new Image()
  const json = { 'sticker-pack-id': 'professor-xmd-avatar', 'sticker-pack-name': packname, 'sticker-pack-publisher': author, 'emojis': categories, 'is-avatar-sticker': 1, ...extra }
